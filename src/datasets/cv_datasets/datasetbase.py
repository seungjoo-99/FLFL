...
class BasicDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.pseudo_targets = np.full_like(targets, -1)
        # self.neg_pseudo_targets = np.full_like(targets, -1)
        self.fix_pseudo_targets = np.full_like(targets, -1)
        ...
        if not check_algorithm('simclrmatch'):
            self.simclr_transform = None
    ...
    def __sample__(self, idx):
        ...
        if self.pseudo_targets is None:
            pseudo_target = None
        else:
            pseudo_target_ = self.pseudo_targets[idx]
            pseudo_target = pseudo_target_ if not self.onehot else get_onehot(self.num_classes, pseudo_target_)

        # set augmented images
        img = self.data[idx]
        return img, target, pseudo_target
    ...
    def __getitem__(self, idx):
        ...
        """
        img, target, pseudo_target = self.__sample__(idx)
        ...
        if self.transform is None:
            ...
            if not self.is_ulb:
                ...
                if self.alg == 'fullysupervised' or self.alg == 'supervised':
                    return {'idx_ulb': idx, 'y_ulb': target}
                ...
                elif check_algorithm('flfl'):
                    fix_pseudo_target_ = self.fix_pseudo_targets[idx]
                    fix_pseudo_target = fix_pseudo_target_ if not self.onehot else get_onehot(self.num_classes, fix_pseudo_target_)
                    return {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s': self.strong_transform(img), 'y_ulb': target, 'pseudo_y_ulb': pseudo_target, 'fix_pseudo_y_ulb': fix_pseudo_target} 
                else:
                    return {'idx_ulb': idx, 'x_ulb_w': img_w, 'x_ulb_s': self.strong_transform(img), 'y_ulb': target, 'pseudo_y_ulb': pseudo_target}
