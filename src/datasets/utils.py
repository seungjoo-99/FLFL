...
def split_ssl_data(data, targets, num_classes,
    ...
    lb_idx, ulb_idx = sample_labeled_unlabeled_data(targets, num_classes, 
                                                    lb_num_labels, ulb_num_labels,
                                                    lb_imbalance_ratio, ulb_imbalance_ratio, load_exist)
...
def sample_labeled_unlabeled_data(target, num_classes,
    ...
    seed = cfg['seed']
    dump_dir = os.path.join(base_dir, 'data', data_name, 'labeled_idx')
    os.makedirs(dump_dir, exist_ok=True)
    if not ulb_num_labels:
        ulb_num_labels = len(target) - lb_num_labels
    lb_dump_path = os.path.join(dump_dir, f'lb_{lb_num_labels}_{lb_imbalance_ratio}_seed{seed}_idx.npy')
    ulb_dump_path = os.path.join(dump_dir, f'ulb_{ulb_num_labels}_{ulb_imbalance_ratio}_seed{seed}_idx.npy')
    ...
    samples_per_class = np.unique(target, return_counts=True)[1].tolist()
    ...
    if ulb_imbalance_ratio == 1.0:
        # balanced setting
        ulb_samples_per_class = [total_cnt - lb_cnt for total_cnt, lb_cnt in zip(samples_per_class, lb_samples_per_class)]
    else:
        # imbalanced setting
...
def check_dataset_split(dataset, split_name):
    return split_name in dataset and dataset[split_name] is not None and len(dataset[split_name]) > 0
